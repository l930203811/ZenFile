...
abstract class RemoteClient {
  ...
  /// 底层连接是否仍然可用。
  ///
  /// 默认返回 true：WebDAV 这类无状态 HTTP 协议每次请求独立建连，「永远可用」。
  /// 有长会话的协议（SMB / SFTP / FTP）**必须覆盖**它去查原生会话的真实状态——
  /// 应用切后台后 socket 会被系统回收、服务器也会因空闲断开，此时 Dart 侧保存的
  /// 「已连接」标记仍然为 true（假连接），用户的下一次操作必然失败。
  ///
  /// 约定：本方法不得抛异常，探测失败一律返回 false（由调用方决定是否重建）。
  Future<bool> checkAlive() async => true;
