...
class _RemoteExplorerScreenState extends State<RemoteExplorerScreen> {
  ...
  void _navigateTo(RemoteFileItem item) {
    if (item.isDirectory) {
      ...
      if (isVideo || isAudio || isText) {
        _autoCacheAndPlay(item, isVideo: isVideo, isAudio: isAudio, isText: isText);
      } else {
  ...
  /// 自动下载缓存并播放媒体文件或打开文本文件
  Future<void> _autoCacheAndPlay(RemoteFileItem item, {bool isVideo = false, bool isAudio = false, bool isText = false}) async {
    if (_client == null) return;
    
    try {
      // 获取本地缓存目录（优先使用应用私有缓存目录，避免存储权限问题）
      Directory? baseCacheDir;
      try {
        baseCacheDir = Directory('/storage/emulated/0/Download/ZenFile_Remote/cache');
        if (!baseCacheDir.existsSync()) baseCacheDir.createSync(recursive: true);
      } catch (_) {
        // 如果外部存储不可用，使用应用私有目录
        final appDir = await getApplicationDocumentsDirectory();
        baseCacheDir = Directory(p.join(appDir.path, 'ZenFile_Remote', 'cache'));
        if (!baseCacheDir.existsSync()) baseCacheDir.createSync(recursive: true);
      }
      
      final localPath = p.join(baseCacheDir.path, widget.connection.name, item.path.replaceAll('/', '_'));
      ...
      if (needsDownload) {
        setState(() {
          ...
          _transferLabel = isText ? '正在下载文本...' : '正在缓存媒体...';
        });
      ...
      if (mounted) {
        setState(() => _isTransferring = false);
        _showSnack('缓存失败：$e', isError: true);
      }
    }
  }
  ...
  Future<void> _pasteRemoteClipboard() async {
    ...
    for (final item in clipItems) {
      ...
      setState(() {
        ...
        _transferProgress = 0.0;
        _transferFileName = item.name;
        _transferLabel = isCut ? '正在移动...' : '正在复制...';
      });
  ...
  Future<void> _renameRemoteItem(RemoteFileItem item) async {
    ...
    final newName = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('重命名', style: TextStyle(fontWeight: FontWeight.bold)),
        content: TextField(
  ...
  Future<void> _deleteItem(RemoteFileItem item) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          title: const Text('删除项目', style: TextStyle(fontFamily: 'LexendDeca', fontWeight: FontWeight.bold)),
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          item.isDirectory ? '远程目录' : item.formattedSize,
              ...
              // Rename
              _buildActionTile(
                ctx, icon: Broken.edit, label: '重命名',
              ...
              if (!item.isDirectory)
                _buildActionTile(
                  ctx, icon: Icons.download_for_offline_rounded, label: '复制到本地设备',
                  subtitle: '下载文件到本地剪贴板',
                  color: const Color(0xFF0D9488),
  ...
  Widget build(BuildContext context) {
    ...
    if (isSaf) {
      ...
      pathNodes = relativePath.isEmpty
          ? ['根目录']
          : ['根目录', ...relativePath.split('/').where((n) => n.isNotEmpty)];
    }
              ...
              if (_isSelectionMode) ...[
                ...
                if (hasRemoteClipboard)
                  IconButton(
                    ...
                    tooltip: provider.isCut ? '移动到此处' : '粘贴远程剪贴板',
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    Text('连接已断开', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                                ...
                                trailing: _isSelectionMode
                                    ...
                                    : PopupMenuButton<String>(
                                        ...
                                        itemBuilder: (context) => [
                                          ...
                                          _popItem('rename', Broken.edit, '重命名', const Color(0xFF0D9488)),
                                          if (hasRemoteClipboard)
                                            _popItem('paste', Icons.content_paste_rounded, '粘贴到此处', const Color(0xFF0D9488)),
                                          const PopupMenuDivider(),
