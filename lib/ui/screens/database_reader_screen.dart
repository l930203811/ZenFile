...
class _DatabaseReaderScreenState extends State<DatabaseReaderScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _buildBrowseTab(ThemeData theme) {
    if (_tables.isEmpty) {
      return Center(
        child: Text('此数据库中未找到表。', style: TextStyle(color: theme.colorScheme.onSurface.withOpacity(0.5))),
      );
    }
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 8),
                          Text('未找到行', style: TextStyle(color: theme.colorScheme.onSurface.withOpacity(0.5))),
                        ],
                      ),
                    )
                  : Scrollbar(
  ...
  Widget _buildSchemaTab(ThemeData theme) {
    if (_schemaColumns.isEmpty) {
      return Center(
        child: Text('未加载结构详情。', style: TextStyle(color: theme.colorScheme.onSurface.withOpacity(0.5))),
      );
    }
              ...
              children: [
                Padding(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text('SQL 编辑器', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: theme.colorScheme.primary)),
                      Row(
                        children: [
                          TextButton(
                            style: TextButton.styleFrom(visualDensity: VisualDensity.compact),
                            child: const Text('SELECT模板', style: TextStyle(fontSize: 12)),
                            onPressed: () {
        ...
        Expanded(
          child: _isSqlRunning
              ...
              : _sqlResultRows.isEmpty
                  ? Center(
                      child: Text(
                        _sqlErrorMessage == null ? '执行SELECT查询以查看结果。' : '执行查询出错。',
