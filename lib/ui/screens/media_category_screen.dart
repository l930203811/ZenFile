...
enum _SyncMenuAction { toggleAuto, backupNow }
...
class _MediaCategoryScreenState extends State<MediaCategoryScreen>
  ...
  bool _showCloudBadge(String path) =>
      ...
      CategorySyncService.isSyncedLocalPath(path);
  ...
  bool _belongsToFolder(String itemPath, String folderPath) {
    ...
    if (itemPath.startsWith('remote://')) {
      final itemFolderName = path_helper.basename(
        MediaProvider.parentOfPath(itemPath),
      );
      final folderName = path_helper.basename(folderPath);
      if (itemFolderName.isNotEmpty && itemFolderName == folderName)
        return true;
    }
    return false;
  }
  ...
  Future<void> _maybeAutoSync() async {
    ...
    final remotePaths = customPaths
        .where((p) => p.startsWith('remote://'))
        .toList();
  ...
  Future<List<CategorySyncPair>> _buildSyncPairs(
  ...
  String _mirrorRoot(List<String> files, {required String fallback}) {
    ...
    if (files.every(
      (f) => f.startsWith('/storage/emulated/0/') || f == '/storage/emulated/0',
    )) {
  ...
  List<String> _collectCategoryFiles([String? scopeDir]) {
    ...
    if (scopeDir != null && scopeDir.isNotEmpty) {
      ...
      result = result
          .where((p) => p == scopeDir || p.startsWith(prefix))
          .toList();
    }
    return result;
  }
  ...
  void initState() {
    ...
    _showRemoteFiles = PreferencesService.getShowRemoteFilesInCategory(
      widget.mediaType.name,
    );
    ...
    if (widget.folderPath != null &&
        MediaProvider.isRemotePath(widget.folderPath!)) {
  ...
  Future<void> _handleCopyCut(bool isCut) async {
    ...
    // 区分远程与本地选中项。
    final remoteSelected = _selectedFilePaths
        .where((p) => p.startsWith('remote://'))
        .toList();
    final localSelected = _selectedFilePaths
        .where((p) => !p.startsWith('remote://'))
        .toList();
    ...
    if (remoteSelected.isNotEmpty) {
      ...
      if (byConn.length == 1) {
        final conn = FileManagerProvider.connectionForRemotePath(
          remoteSelected.first,
        );
        if (conn != null) {
    ...
    if ((paths.isNotEmpty || remoteSelected.isNotEmpty) && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            isCut
                ? L10n.of(context).ui_cut_count(total)
                : L10n.of(context).ui_copied_count(total),
          ),
        ),
      );
      _clearSelection();
  ...
  Future<void> _handleDelete() async {
    ...
    if (confirm == true && mounted) {
      ...
      final filePaths = _selectedFilePaths
          .where((p) => !p.startsWith('remote://'))
          .toList();
      final remotePaths = _selectedFilePaths
          .where((p) => p.startsWith('remote://'))
          .toList();
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(L10n.of(context).count2(count))));
        _clearSelection();
      }
    }
  }
  ...
  Future<void> _handlePaste() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          L10n.of(context).pastedcountdestdir(pastedCount, destDir),
        ),
      ),
    );
  ...
  Future<void> _handleBatchRename() async {
    ...
    if (filePaths.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(L10n.of(context).msg3ad97542)));
      }
      return;
    }
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text(
            _isSelectionMode
                ? L10n.of(context).ui_selected_count(
                    _selectedFilePaths.length + _selectedAssetIds.length,
                  )
                : _title,
          ),
          ...
          actions: [
            if (_isSelectionMode)
                      ...
                      switch (action) {
                        ...
                        case _ViewMenuAction.togglePlayerController:
                          if (widget.mediaType == MediaType.audios) {
                            ...
                            await PreferencesService.saveShowResumeAudio(
                              newValue,
                            );
                            setState(() => _showResumeAudio = newValue);
                          } else if (widget.mediaType == MediaType.videos) {
                            final newValue = !_showResumeVideo;
                            await PreferencesService.saveShowResumeVideo(
                              newValue,
                            );
                        ...
                        : () async {
                            ...
                            if (mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                    L10n.of(context).ui_refresh_done,
                                  ),
              ...
              if (_supportsRemoteSync) _buildBackupButton(theme),
                    ...
                    if (folders.isEmpty) {
                      ...
                      final entries = folders.entries
                          ...
                          .where((e) {
                            ...
                            return _scopeFilter == _ScopeFilter.remote
                                ? isRemote
                                : !isRemote;
                          })
                          .toList();
                      entries.sort((a, b) {
                        ...
                        return path_helper
                            .basename(a.key)
                            .compareTo(path_helper.basename(b.key));
                      });
                  ...
                  switch (widget.mediaType) {
                    case MediaType.images:
                      ...
                      final list = widget.folderPath != null
                          ? <dynamic>[
                              ...provider.images.where(
                                (f) => _belongsToFolder(
                                  f.path,
                                  widget.folderPath!,
                                ),
                              ),
                              ...provider.screenshots.where(
                                (f) => _belongsToFolder(
                                  f.path,
                                  widget.folderPath!,
                                ),
                      ...
                      final filtered = _filterByScope(
                        list,
                        (dynamic f) => (f as FileSystemEntity).path,
                      );
                      ...
                      final list = widget.folderPath != null
                          ? provider.audios.where((s) {
                              if (s.data.isNotEmpty) {
                                return _belongsToFolder(
                                  s.data,
                                  widget.folderPath!,
                                );
                              }
                              // data 为空（该设备 on_audio_query 特例）：归入「未分类音频」虚拟文件夹
                              return widget.folderPath ==
                                  MediaProvider.unknownAudioFolder;
                            }).toList()
                          : provider.audios;
  ...
  int _fileSizeOf(FileSystemEntity entity) {
    ...
    final cached = Provider.of<MediaProvider>(
      context,
      listen: false,
    ).nonMediaSizes[p];
            ...
            PageRouteBuilder(
              pageBuilder: (context, animation, secondaryAnimation) =>
                  AudioPlayerScreen(
                    audioPath: path,
                    title: audio.title,
                    artist: _isUnknownArtist(audio.artist)
                        ? L10n.of(context).msg5e32276d
                        : audio.artist!,
                    allSongs: audios,
                    initialIndex: index,
                  ),
  ...
  Widget _buildAudioGridTile(
    ...
    final artist = _isUnknownArtist(audio.artist)
        ? L10n.of(context).msg5e32276d
        : audio.artist!;
      ...
      return GridView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return _SelectedFrame(
            ...
            isList: false,
            theme: theme,
            child: _buildGenericFileGridTile(
              file,
              theme,
              isSelected,
              isDateWise,
              ...
              child: Row(
                children: [
                  Expanded(
                    child: InkWell(
                      onTap: () =>
                          setState(() => _scopeFilter = _ScopeFilter.local),
                      borderRadius: BorderRadius.circular(12),
                      child: Container(
                        alignment: Alignment.center,
    ...
    return PopupMenuButton<_SyncMenuAction>(
      ...
      onSelected: (action) async {
        if (action == _SyncMenuAction.toggleAuto) {
          ...
          await CategorySyncService.setAutoSyncEnabled(
            _categoryLabel,
            newValue,
          );
          ...
          children: [
            Expanded(
              child: InkWell(
                onTap: () {
                  setState(() => _showFoldersMode = false);
                  PreferencesService.savePreferFoldersInMedia(
                    widget.mediaType.name,
                    false,
                  );
                },
        ...
        builder: (context, playingSnapshot) {
          ...
          return StreamBuilder<MediaItem?>(
            ...
            builder: (context, itemSnapshot) {
              ...
              final savedPosition = savedMs != null
              ...
              if (isCurrentTrack && isPlaying && duration > Duration.zero) {
                ...
                subtitle =
                    '${formatDuration(position)} / ${formatDuration(duration)} · ${L10n.of(context).ui_resume_playback}';
              } else if (savedPosition.inSeconds > 0) {
                subtitle =
                    '${L10n.of(context).ui_resume_playback} · ${formatDuration(savedPosition)}';
              } else {
                subtitle = _isUnknownArtist(artist)
                    ? L10n.of(context).msg5e32276d
                    : artist;
              }
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Text(
                                    isCurrentTrack && isPlaying
                                        ? L10n.of(context).ui_now_playing
  ...
  void _openAudioPlayerFromResume(String path, String title, String artist) {
    final handler = getAudioHandler();
    final isSamePlaying =
        handler.isPlayingPath(path) && handler.hasActivePlayer;
  ...
  Future<void> _toggleResumePlayback(
    String path,
    String title,
    String artist,
  ) async {
...
class _RemoteImageThumbState extends State<_RemoteImageThumb> {
  ...
  Future<void> _load() async {
    final local = await FileManagerProvider.downloadRemoteFileToCache(
      widget.path,
    );
    if (mounted) {
      setState(() {
          ...
          child: isSvg
              ? FutureBuilder<File?>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    return const _ThumbnailShimmerPlaceholder(
                      key: ValueKey('shimmer'),
                    );
                  },
                )
  ...
  Future<void> _loadThumbnail() async {
    try {
      if (widget.asset != null) {
        ...
        final data = await MediaThumbnailService.generateVideoThumbnail(
          widget.filePath!,
        );
        if (mounted) {
          setState(() {
...
class _LocalVideoTileState extends State<_LocalVideoTile> {
  ...
  Future<void> _load() async {
    ...
    try {
      final bytes = await MediaThumbnailService.generateVideoThumbnail(
        widget.filePath,
      );
          ...
          child: Stack(
            ...
            children: [
              ...
              if (folderPath.startsWith('remote://') ||
                  CategorySyncService.hasSyncedFileUnder(folderPath))
                Positioned(
                  top: 8,
                  right: 8,
                  child: Container(
