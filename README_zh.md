...
- **单窗口激活标签背景高亮**：当前激活（已打开）的标签页按钮背景现在会高亮，之前仅标题文字高亮、背景无区分。
- **双窗口多标签显示 / 切换重构**：A 在左窗口、B 在右窗口；新建或切换到其他标签页时，已激活标签保持所在窗口位置不变，被顶替的始终是未激活一侧窗口。
- **双窗口可关闭最后两个标签**：仅剩两个标签页时也能关闭任意一个，关闭后自动新建一个标签顶替被关闭位置，保证两侧窗口始终有内容。
- **双窗口远程标签标题**：固定显示已保存的远程客户端名称（不再随打开的目录变化）。
- **远程标签云徽图标**：远程标签页标题左侧图标改为云徽，方便一眼辨别远程 / 本地。
