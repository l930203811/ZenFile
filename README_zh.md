...
- SFTP 传输优化：下载使用 256KB chunk + 64 pending（在途窗口约 16MB），上传使用多缓冲并发写入（在途约 4MB），显著缓解 RTT 限制，传输速率大幅提升。

### 🐛 问题修复

- 修复从远程剪切/移动到本地（或本地剪切/移动到远程）后，来源目录仍显示原文件、必须手动刷新才消失的问题。
- 修复音频播放器「后台播放」开关在菜单中点击后状态不即时刷新（需重开菜单才更新）的问题。
- 修复安卓 13 及以上版本通知栏不显示媒体播放控制按钮的问题。通过本地 fork `audio_service` 0.18.18 并补丁：所有 Android 版本均设置紧凑视图控制按钮，且任意非空闲播放态均激活 `MediaSession`/前台服务，确保 13+ 媒体卡片正常出现。
